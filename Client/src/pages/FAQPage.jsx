  ...
  return (
    <div className="border-b border-white/8 last:border-0">
      ...
      >
        ...
        <ChevronDown
          size={18}
          className={`text-white/30 shrink-0 mt-0.5 transition-transform duration-200 ${open ? 'rotate-180' : ''}`}
        />
      </button>
      {open && (
        <p className="text-sm text-white/50 leading-relaxed pb-5 pr-8">{a}</p>
      )}
    </div>
  )
}
        ...
        <div className="py-12" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.03)' }}>
          <div className="max-w-3xl mx-auto px-4 sm:px-6">
            <div className="flex items-center gap-2 text-xs text-white/30 mb-4">
              ...
              <span>/</span>
              <span className="text-white/50">{t('nav.faq')}</span>
            </div>
            <p className="text-xs font-black text-gold tracking-[0.25em] uppercase mb-2">{t('faq.badge')}</p>
            <h1 className="font-heading text-4xl font-bold text-white">{t('faq.title')}</h1>
          </div>
        </div>

        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
          <div className="rounded-2xl px-6 divide-y" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', divideColor: 'rgba(255,255,255,0.08)' }}>
